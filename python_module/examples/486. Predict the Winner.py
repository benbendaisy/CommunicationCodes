...
class Solution:
    ...
    def PredictTheWinner1(self, nums: List[int]) -> bool:
        @lru_cache(None)
        def predict_the_winner(left, right, player1, player2, is_player1):
            if left > right:
                return player1 >= player2
            if is_player1:
                return predict_the_winner(left + 1, right, player1 + nums[left], player2, False) or predict_the_winner(left + 1, right - 1, player1 + nums[right], player2, False)
