...
class Solution:
    ...
    def getOrder(self, tasks: List[List[int]]) -> List[int]:
        ...
        while task_index < len(tasks) or next_task:
            if not next_task and cur_time < sorted_tasks[task_index][0]:
                # When the heap is empty, try updating curr_time to next task's enqueue time.
                cur_time = sorted_tasks[task_index][0]
            # Push all the tasks whose enqueueTime <= currtTime into the heap.
            while task_index < len(sorted_tasks) and cur_time >= sorted_tasks[task_index][0]:
