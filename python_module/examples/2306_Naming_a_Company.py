...
class Solution:
    ...
    def distinctNames(self, ideas: List[str]) -> int:
        ...
        init_groups = [set() for _ in range(26)]
        for idea in ideas:
            init_groups[ord(idea[0]) - ord('a')].add(idea[1:])
        ...
        for i in range(25):
            for j in range(i + 1, 26):
                ...
                num_of_mutual = len(init_groups[i] & init_groups[j])
                ...
                ans += 2 * (len(init_groups[i]) - num_of_mutual) * (len(init_groups[j]) - num_of_mutual)
        return ans
