class Solution:
    ...
    """
    def minimumTime1(self, jobs: List[int], workers: List[int]) -> int:
        """
        Time Limit Exceeded
        """
        n = len(jobs)
        ...
        def helper(idx: int):
            if idx == n:
                self.min_days = min(self.min_days, max(days))
                return
            
            for i in range(n):
                if days[i] == 0:
                    working_days = math.ceil(jobs[idx] / workers[idx])
                    if working_days > self.min_days:
                        continue
                    days[i] = working_days
                    helper(idx + 1)
                    days[i] = 0
    ...
    def minimumTime2(self, jobs: List[int], workers: List[int]) -> int:
        n = len(jobs)
        jobs.sort(reverse=True)
        workers.sort(reverse=True)
        days = [0] * n
        for i in range(n):
            days[i] = math.ceil(jobs[i] / workers[i])
        return max(days)