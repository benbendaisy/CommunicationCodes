...
public class WordLadderII {
    ...
    //recursive way to get all paths
    public List<List<String>> findLadders(String start, String end, Set<String> dict) {
        ...
        findLaddersHelper(start, end, dict, lls, ls, visited);
        return lls;
    }

    private void findLaddersHelper(String start, String end, Set<String> dict, List<List<String>> lls, List<String> ls, Set<String> visited){
        if(start.equals(end)){
            lls.add(ls);
            return;
        }
        ...
        for(int i = 0; i < len; i++){
            ...
            for(char c = 'a'; c <= 'z'; c++){
                ...
                if(dict.contains(tempStr) && visited.add(tempStr)){
                    ...
                    findLaddersHelper(tempStr, end, dict, lls, newLs, newVisited);
                }
            }
        }
    }

    //bfs way to identify candidates and recursively construct results
    //although the result looks right but it can't pass leetcode as it took too much time
    //to construct result
    public List<List<String>> findLaddersI(String start, String end, Set<String> dict) {
        List<List<String>> lls = new ArrayList<List<String>>();
        if(start == null || end == null || dict == null || dict.size() == 0){
            return lls;
        }
        ...
        while(!queue.isEmpty()){
            ...
            for(int i = 0; i < candidate.length(); i++){
                char[] word = candidate.toCharArray();
                for(char c = 'a'; c <= 'z'; c++){
                    word[i] = c;
                    String tempStr = new String(word);
                    if(tempStr.equals(end)){
                        ...
                        if(map.containsKey(tempStr)){
                            map.get(tempStr).add(candidate);
                        } else {
                            Set<String> newSet = new LinkedHashSet<String>();
                            newSet.add(candidate);
                            map.put(tempStr, newSet);
                        }
                        break;
                    }
                    if(dict.contains(tempStr) && !candidate.equals(tempStr)){
                        if(map.containsKey(tempStr)){
                            map.get(tempStr).add(candidate);
                        } else {
                            Set<String> newSet = new LinkedHashSet<String>();
        ...
        if(hasResult){
            ls.add(end);
            getLadders(map, lls, ls, end, start, level);
        }
        return lls;
    }

    private void getLadders(Map<String, Set<String>> map, List<List<String>> lls, List<String> ls, String str, String start, int level){
        if(!map.containsKey(str) || str.equals(start)){
            lls.add(ls);
        } else if(level > 0) {
            ...
            for(String str1 : set){
                ...
                newLs.add(0, str1);
                level--;
                getLadders(map, lls, newLs, str1, start, level);
            }
        }
    }
